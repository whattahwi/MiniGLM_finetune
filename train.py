...
# data
dataset = 'shediao'
gradient_accumulation_steps = 5 * 8 # 梯度积累步骤 used to simulate(模拟) larger batch sizes
...
# learning rate decay(衰变) settings
...
if init_from == 'scratch':
    ...
    # force these config attributes to be equal otherwise we can't even resume(恢复) training
...
# helps estimate an arbitrarily accurate loss over either split(裂) using many batches
@torch.no_grad()
def estimate_loss():
    out = {}
    model.eval()
...
X, Y, loss_mask = get_batch('train') # fetch(取) the very first batch
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        # immediately async(异步) prefetch next batch while model is doing the forward pass on the GPU
